...
finish() {
  ...
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.utility768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_UTILITY768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.utility_probe_run \
    --source "${BABEL_UTILITY_SOURCE:-checkpoints/babel_path768}" --out "$run" --batch "${BABEL_UTILITY_BATCH:-128}"
fi
