...
run="${BABEL_BAR_SEMANTICS_RUN:-checkpoints/babel_bar_semantics768}"
log="${BABEL_BAR_SEMANTICS_LOG:-logs/babel_bar_semantics768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_bar_semantics768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_BAR_SEMANTICS768.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.bar_semantics_run \
    --source "${BABEL_BAR_SEMANTICS_SOURCE:-checkpoints/babel_control600_v2}" --warm-source "${BABEL_BAR_SEMANTICS_WARM_SOURCE:-checkpoints/babel_history_query768}" --out "$run" --jobs "${BABEL_BAR_SEMANTICS_JOBS:-2}" --micro "${BABEL_BAR_SEMANTICS_MICRO:-16}"
fi
