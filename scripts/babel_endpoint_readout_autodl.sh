...
run="${BABEL_ENDPOINT_RUN:-checkpoints/babel_endpoint_readout}"
log="${BABEL_ENDPOINT_LOG:-logs/babel_endpoint_readout.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_endpoint_readout_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.endpoint_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_ENDPOINT_READOUT.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.endpoint_readout_audit \
    --source "${BABEL_ENDPOINT_SOURCE:-checkpoints/babel_bar_alignment}" --out "$run"
fi
