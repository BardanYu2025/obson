...
run="${BABEL_HOLDOUT_RUN:-checkpoints/babel_state_holdout512}"
log="${BABEL_HOLDOUT_LOG:-logs/babel_state_holdout512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|audit|export) ;; *) echo 'Usage: bash scripts/babel_state_holdout_autodl.sh [all|audit|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.holdout_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_STATE_HOLDOUT.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.state_holdout "$mode" \
    --transfer "${BABEL_HOLDOUT_TRANSFER:-checkpoints/babel_state_transfer512}" --out "$run" \
    --registry "${BABEL_HOLDOUT_REGISTRY:-checkpoints}" --root "${BABEL_ROOT:-/root/autodl-tmp/data/contracts}" \
    --streams "${BABEL_HOLDOUT_STREAMS:-32}" --cap "${BABEL_HOLDOUT_CAP:-256}"
fi
