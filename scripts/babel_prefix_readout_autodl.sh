...
run="${BABEL_PREFIX_RUN:-checkpoints/babel_prefix_readout512}"
log="${BABEL_PREFIX_LOG:-logs/babel_prefix_readout512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_prefix_readout_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.prefix_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_PREFIX_READOUT.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.prefix_readout_benchmark "$mode" \
    --source "${BABEL_PREFIX_SOURCE:-checkpoints/babel_sampling512}" --out "$run" \
    --jobs "${BABEL_PREFIX_JOBS:-2}"
fi
