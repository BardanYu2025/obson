...
run="${BABEL_CROSS_PERIOD_RUN:-checkpoints/babel_cross_period768}"
log="${BABEL_CROSS_PERIOD_LOG:-logs/babel_cross_period768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_cross_period768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_CROSS_PERIOD768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.cross_period_run all \
