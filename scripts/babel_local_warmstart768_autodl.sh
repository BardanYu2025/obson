...
run="${BABEL_LOCAL_WARMSTART_RUN:-checkpoints/babel_local_warmstart768}"
log="${BABEL_LOCAL_WARMSTART_LOG:-logs/babel_local_warmstart768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_local_warmstart768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.local_warmstart768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_LOCAL_WARMSTART768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.local_warmstart_run all \
    --source "${BABEL_LOCAL_WARMSTART_SOURCE:-checkpoints/babel_cross_period768_v2_recheck}" --out "$run" \
    --jobs "${BABEL_LOCAL_WARMSTART_JOBS:-1}"
fi
