...
run="${BABEL_TEACHER_RUN:-checkpoints/babel_pca_teacher512}"
log="${BABEL_TEACHER_LOG:-logs/babel_pca_teacher512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_pca_teacher_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.teacher_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_PCA_TEACHER.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.pca_teacher_benchmark "$mode" \
    --source "${BABEL_TEACHER_SOURCE:-checkpoints/babel_architecture512}" --out "$run" \
    --parent-epochs "${BABEL_TEACHER_PARENT_EPOCHS:-200}" \
    --branch-epochs "${BABEL_TEACHER_BRANCH_EPOCHS:-100}" --batch "${BABEL_TEACHER_BATCH:-128}" \
    --micro "${BABEL_TEACHER_MICRO:-64}" --jobs "${BABEL_TEACHER_JOBS:-2}"
fi
