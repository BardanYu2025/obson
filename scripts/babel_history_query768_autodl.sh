...
run="${BABEL_HISTORY_QUERY_RUN:-checkpoints/babel_history_query768}"
log="${BABEL_HISTORY_QUERY_LOG:-logs/babel_history_query768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_history_query768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_HISTORY_QUERY768.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.history_query_run \
    --source "${BABEL_HISTORY_QUERY_SOURCE:-checkpoints/babel_control600_v2}" --out "$run" --jobs "${BABEL_HISTORY_QUERY_JOBS:-1}" --micro "${BABEL_HISTORY_QUERY_MICRO:-16}"
fi
