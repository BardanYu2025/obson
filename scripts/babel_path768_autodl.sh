...
run="${BABEL_PATH_RUN:-checkpoints/babel_path768}"
log="${BABEL_PATH_LOG:-logs/babel_path768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_path768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.path768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_PATH768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.path_feature_benchmark all \
    --source "${BABEL_PATH_SOURCE:-checkpoints/babel_bar_alignment}" --out "$run" \
    --jobs "${BABEL_PATH_JOBS:-2}" --micro "${BABEL_PATH_MICRO:-64}"
fi
