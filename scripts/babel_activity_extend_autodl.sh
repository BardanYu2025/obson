...
run="${BABEL_EXTEND_RUN:-checkpoints/babel_activity_alignment100}"
...
finish() {
  ...
  cp docs/BABEL_ACTIVITY_EXTEND.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.activity_extend "$mode" \
    --parent "${BABEL_ALIGNMENT_PARENT:-checkpoints/babel_activity_alignment512}" --out "$run" \
    --epochs "${BABEL_EXTEND_EPOCHS:-100}" --selection-every "${BABEL_EXTEND_SELECTION_EVERY:-10}" \
    --jobs "${BABEL_EXTEND_JOBS:-2}"
fi
