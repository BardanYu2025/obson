...
run="${BABEL_TRANSFER_RUN:-checkpoints/babel_state_transfer512}"
log="${BABEL_TRANSFER_LOG:-logs/babel_state_transfer512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|export) ;; *) echo 'Usage: bash scripts/babel_state_transfer_autodl.sh [all|evaluate|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.transfer_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_STATE_TRANSFER.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.state_transfer "$mode" \
    --parent "${BABEL_TRANSFER_PARENT:-checkpoints/babel_price_readapt512}" --out "$run" \
    --epochs "${BABEL_TRANSFER_EPOCHS:-100}" --batch "${BABEL_TRANSFER_BATCH:-256}" \
    --jobs "${BABEL_TRANSFER_JOBS:-2}" --root "${BABEL_ROOT:-data/contracts}"
fi
