...
run="${BABEL_CROSS_PERIOD_RUN:-checkpoints/babel_cross_period_audit}"
log="${BABEL_CROSS_PERIOD_LOG:-logs/babel_cross_period_audit.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_cross_period_audit_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.cross_period_audit.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_CROSS_PERIOD_AUDIT.md "$stage/$name/audit_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.cross_period_audit \
    --root "${BABEL_DATA_ROOT:-/root/autodl-tmp/data/contracts}" --out "$run"
fi
