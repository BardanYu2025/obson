...
run="${BABEL_SHARED_RUN:-checkpoints/babel_shared_readout512}"
log="${BABEL_SHARED_LOG:-logs/babel_shared_readout512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_shared_readout_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.shared_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_SHARED_READOUT.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.shared_readout_benchmark "$mode" \
    --source "${BABEL_SHARED_SOURCE:-checkpoints/babel_prefix_readout512}" --out "$run" \
    --jobs "${BABEL_SHARED_JOBS:-2}"
fi
