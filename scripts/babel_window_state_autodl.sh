...
run="${BABEL_STATE_RUN:-checkpoints/babel_window_state}"
log="${BABEL_STATE_LOG:-logs/babel_window_state.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_window_state_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.state_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_WINDOW_STATE.md "$stage/$name/experiment_notes.md"
  ...
  if [[ "$run_status" == complete && -f "$run/bundle/validation.json" ]]; then
    bundle_stage=$(mktemp "$download/.state_bundle.XXXXXX")
    tar -czf "$bundle_stage" -C "$run" bundle
    mv "$bundle_stage" "$download/${name}_bundle.tar.gz"
    echo "Validated portable bundle: $download/${name}_bundle.tar.gz"
  fi
  exit "$status"
}
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.window_state_delivery \
    --source "${BABEL_STATE_SOURCE:-checkpoints/babel_endpoint_readout}" --out "$run" --root "${BABEL_STATE_ROOT:-/root/autodl-tmp/data/contracts}"
fi
