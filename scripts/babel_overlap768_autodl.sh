...
run="${BABEL_OVERLAP_RUN:-checkpoints/babel_overlap768}"
log="${BABEL_OVERLAP_LOG:-logs/babel_overlap768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_overlap768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.overlap768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_OVERLAP768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.overlap_diagnostic_run \
    --source "${BABEL_OVERLAP_SOURCE:-checkpoints/babel_path768}" --out "$run" --batch "${BABEL_OVERLAP_BATCH:-128}"
fi
