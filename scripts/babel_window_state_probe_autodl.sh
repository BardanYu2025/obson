...
run="${BABEL_STATE_PROBE_RUN:-checkpoints/babel_window_state_probe}"
log="${BABEL_STATE_PROBE_LOG:-logs/babel_window_state_probe.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_window_state_probe_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_WINDOW_STATE_PROBE.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.window_state_probe_run \
    --source "${BABEL_STATE_PROBE_SOURCE:-checkpoints/babel_window_state}" --out "$run" --batch "${BABEL_STATE_PROBE_BATCH:-256}"
fi
