...
run="${BABEL_ACTIVITY_RUN:-checkpoints/babel_activity512}"
log="${BABEL_ACTIVITY_LOG:-logs/babel_activity512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|export) ;; *) echo 'Usage: bash scripts/babel_activity_autodl.sh [all|evaluate|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_ACTIVITY_ABLATION.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.activity_ablation "$mode" \
    ...
    --epochs "${BABEL_ACTIVITY_EPOCHS:-30}" --streams "${BABEL_ACTIVITY_STREAMS:-64}" --jobs "${BABEL_ACTIVITY_JOBS:-2}"
fi
