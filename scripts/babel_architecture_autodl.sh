...
run="${BABEL_ARCH_RUN:-checkpoints/babel_architecture512}"
log="${BABEL_ARCH_LOG:-logs/babel_architecture512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_architecture_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.architecture_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_ARCHITECTURE.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.architecture_benchmark "$mode" \
    --transfer "${BABEL_ARCH_TRANSFER:-checkpoints/babel_state_transfer512}" \
    --cross-run "${BABEL_ARCH_CROSS:-checkpoints/babel_state_holdout512}" --out "$run" \
    --epochs "${BABEL_ARCH_EPOCHS:-200}" --batch "${BABEL_ARCH_BATCH:-128}" \
    --micro "${BABEL_ARCH_MICRO:-64}" --jobs "${BABEL_ARCH_JOBS:-2}"
fi
