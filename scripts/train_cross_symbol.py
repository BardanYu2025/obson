...
def main() -> None:
    ...
    ap.add_argument("--cross-daily-bars", type=int, default=0,
                    help="跨品种日线 variate token：每品种取 N 根已收盘日K（0=关闭，建议 20）；"
                         "品种token间一层双向attention捕获板块联动（iTransformer式）")
    ap.add_argument("--sector-tokens", action="store_true",
                    help="板块分组 token（iTransformer variate 的轻量版）：组内均值替代跨品种"
                         "attention，零新增时序参数，方差远小于 --cross-daily-bars 的完整版；"
                         "需配合 --cross-daily-bars 指定窗口（建议 20）和 --daily-bars>0")
    ...
    total_counts = None
    # 跨品种日线原料（全品种一次性聚合，所有格子共享同一份因果日K字典）
    cross_daily = build_cross_daily() if args.cross_daily_bars > 0 else None
    if cross_daily is not None:
        print(f"  跨品种日线: {len(cross_daily)} 个品种已聚合（variate token 原料）")
        ...
        for period in periods:
            ...
            try:
                train_ds, val_ds, test_ds = build_datasets(
                    ...
                    cross_daily_bars=args.cross_daily_bars, **extra_kwargs,
    ...
    model_config = KLineConfig(
        ...
        cross_daily_bars=args.cross_daily_bars,  # 跨品种 variate token
        cross_symbol_ids=(sorted(cross_daily.keys()) if cross_daily is not None else None),
        sector_groups=(
            [[SYMBOLS[c] for c in grp if c in SYMBOLS] for grp in [
                ["rb", "hc", "i", "j", "jm"],   # 黑色系
                ["p", "y", "m"],                # 油脂油料
                ["TA", "MA"],                   # 化工
                ["cu", "ag"],                   # 有色/贵金属
                ["sr"],                         # 软商品
            ]] if args.sector_tokens else None
        ),
    )
            ...
            for batch in loader:
                ...
                if args.task == "classify":
                    ...
                    out = model(
                        ...
                        cross_ctx=cross_ctx, cross_mask=cross_mask,
                        fine_ctx=fine_ctx,
                    )
