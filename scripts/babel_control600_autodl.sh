...
finish() {
  ...
  cp docs/BABEL_CONTROL600.md "$stage/$name/experiment_notes.md"
  ...
  if [[ "$run_status" == complete && -f "$run/model_review.html" ]]; then
    cp "$run/model_review.html" "$download/${name}_review.html"
    echo "Offline view: $download/${name}_review.html"
  fi
  exit "$status"
}
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.research_state_delivery \
    --source "${BABEL_CONTROL600_SOURCE:-checkpoints/babel_cross_period768_v2_recheck}" --out "$run"
fi
