...
run="${BABEL_ANNEAL_RUN:-checkpoints/babel_pca_anneal512}"
log="${BABEL_ANNEAL_LOG:-logs/babel_pca_anneal512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_pca_anneal_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.anneal_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_PCA_ANNEAL.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.pca_anneal_benchmark "$mode" \
    --source "${BABEL_ANNEAL_SOURCE:-checkpoints/babel_pca_teacher512}" --out "$run" \
    --root "${BABEL_DATA_ROOT:-/root/autodl-tmp/data/contracts}" --jobs "${BABEL_ANNEAL_JOBS:-2}"
fi
