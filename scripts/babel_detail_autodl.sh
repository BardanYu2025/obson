...
run="${BABEL_DETAIL_RUN:-checkpoints/babel_detail512}"
log="${BABEL_DETAIL_LOG:-logs/babel_detail512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|export) ;; *) echo 'Usage: bash scripts/babel_detail_autodl.sh [all|evaluate|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.detail_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_DETAIL_ALIGNMENT.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.detail_alignment "$mode" \
    ...
    --epochs "${BABEL_DETAIL_EPOCHS:-30}" --streams "${BABEL_DETAIL_STREAMS:-64}" --jobs "${BABEL_DETAIL_JOBS:-2}"
fi
