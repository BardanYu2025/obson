...
run="${BABEL_CAPACITY_RUN:-checkpoints/babel_capacity512}"
log="${BABEL_CAPACITY_LOG:-logs/babel_capacity512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|audit|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_capacity_autodl.sh [all|audit|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.capacity_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_NATIVE_CAPACITY.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.capacity_benchmark "$mode" \
    --source "${BABEL_CAPACITY_SOURCE:-checkpoints/babel_architecture512}" --out "$run" \
    --epochs "${BABEL_CAPACITY_EPOCHS:-200}" --batch "${BABEL_CAPACITY_BATCH:-128}" \
    --micro "${BABEL_CAPACITY_MICRO:-64}" --jobs "${BABEL_CAPACITY_JOBS:-2}"
fi
