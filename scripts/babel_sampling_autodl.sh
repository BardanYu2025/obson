...
run="${BABEL_SAMPLING_RUN:-checkpoints/babel_sampling512}"
log="${BABEL_SAMPLING_LOG:-logs/babel_sampling512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|preflight|export) ;; *) echo 'Usage: bash scripts/babel_sampling_autodl.sh [all|evaluate|preflight|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.sampling_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_WINDOW_SAMPLING.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.sampling_benchmark "$mode" \
    --source "${BABEL_SAMPLING_SOURCE:-checkpoints/babel_pca_teacher512}" --out "$run" \
    --coverage "${BABEL_SAMPLING_COVERAGE:-checkpoints/babel_coverage512}" \
    --root "${BABEL_DATA_ROOT:-/root/autodl-tmp/data/contracts}" --jobs "${BABEL_SAMPLING_JOBS:-2}"
fi
