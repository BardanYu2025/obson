...
run="${BABEL_ARDEC_RUN:-checkpoints/babel_ardec512}"
log="${BABEL_ARDEC_LOG:-logs/babel_ardec512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|evaluate|export) ;; *) echo 'Usage: bash scripts/babel_ardec_autodl.sh [all|evaluate|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.ardec_export.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_AR_RECONSTRUCTION.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.ar_reconstruction "$mode" \
    ...
    --epochs "${BABEL_ARDEC_EPOCHS:-60}" --batch "${BABEL_ARDEC_BATCH:-256}" \
    --eval-batch "${BABEL_ARDEC_EVAL_BATCH:-256}" --jobs "${BABEL_ARDEC_JOBS:-2}"
fi
