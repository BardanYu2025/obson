...
run="${BABEL_CONSISTENCY_RUN:-checkpoints/babel_consistency768}"
log="${BABEL_CONSISTENCY_LOG:-logs/babel_consistency768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_consistency768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.consistency768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_CONSISTENCY768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.overlap_consistency_run all \
    --source "${BABEL_CONSISTENCY_SOURCE:-checkpoints/babel_path768}" --out "$run" \
    --reader "${BABEL_CONSISTENCY_READER:-checkpoints/babel_utility768_v2}" \
    --jobs "${BABEL_CONSISTENCY_JOBS:-2}" --micro "${BABEL_CONSISTENCY_MICRO:-64}"
fi
