...
finish() {
  ...
  cp docs/BABEL_DECODER_PARITY768.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.decoder_parity \
    --source "${BABEL_DECODER_PARITY_SOURCE:-checkpoints/babel_control600_v2}" --out "$run"
fi
