...
run="${BABEL_RECAL_RUN:-checkpoints/babel_consistency_readout768}"
log="${BABEL_RECAL_LOG:-logs/babel_consistency_readout768.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-all}"
case "$mode" in all|export) ;; *) echo 'Usage: bash scripts/babel_consistency_readout768_autodl.sh [all|export]' >&2; exit 2 ;; esac
finish() {
  command_code=$?
  trap - EXIT
  mkdir -p "$download"
  stage=$(mktemp -d "$download/.consistency_readout768.XXXXXX")
  name=$(basename "$run")
  mkdir -p "$stage/$name"
  if [[ -d "$run" ]]; then
  ...
  cp docs/BABEL_CONSISTENCY_READOUT768.md "$stage/$name/experiment_notes.md"
  run_status=partial
...
trap finish EXIT
if [[ "$mode" == all ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.consistency_readout \
    --source "${BABEL_RECAL_SOURCE:-checkpoints/babel_consistency768}" --out "$run" \
    --batch "${BABEL_RECAL_BATCH:-128}"
fi
