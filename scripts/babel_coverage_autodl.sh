...
run="${BABEL_COVERAGE_RUN:-checkpoints/babel_coverage512}"
log="${BABEL_COVERAGE_LOG:-logs/babel_coverage512.log}"
download="${BABEL_DOWNLOAD_DIR:-/root/autodl-tmp/download}"
mode="${1:-audit}"
case "$mode" in audit|export) ;; *) echo 'Usage: bash scripts/babel_coverage_autodl.sh [audit|export]' >&2; exit 2 ;; esac
finish() {
  status=$?
  trap - EXIT
  mkdir -p "$download"
  ...
  cp docs/BABEL_COVERAGE_AUDIT.md "$stage/$name/experiment_notes.md"
...
trap finish EXIT
if [[ "$mode" != export ]]; then
  "${PYTHON_BIN:-python}" -m obson.babel.coverage_audit \
    --root "${BABEL_DATA_ROOT:-/root/autodl-tmp/data/contracts}" \
    --source "${BABEL_COVERAGE_SOURCE:-checkpoints/babel_architecture512}" \
    --bank "${BABEL_COVERAGE_BANK:-checkpoints/babel_activity_alignment100}" \
    --long-run "${BABEL_COVERAGE_LONG:-checkpoints/babel_large512_s42}" \
    --cross "${BABEL_COVERAGE_CROSS:-checkpoints/babel_state_holdout512}" --out "$run"
fi
