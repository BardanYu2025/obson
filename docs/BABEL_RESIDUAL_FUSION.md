...
首轮长期向量的AdamW读出明显弱于之前的标准化岭回归，因此不能仅以该较弱基线衡量融合收益。
运行 `bash scripts/babel_fusion_readout_autodl.sh`，只读取feature_cache和已选best.pt，不更新任何神经网络。
五个方案统一用训练集逐维标准化、训练类别平衡权重、验证集选择alpha=[1,10,100]的岭回归。
追加1024维拼接作为保留两份信息的参照，明确不是等维压缩对照。
同时记录训练向量各维标准差和公共均值分量所占能量，辅助判断原读出受尺度/优化影响的可能性。
所有原始重建退化结果仍有效，不因换读出而消失；此检查不重新选择融合checkpoint。
报告自动导出到download/babel_fusion512_readout_audit；无需原数据、GPU编码或重新提取表示。
