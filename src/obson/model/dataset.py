            ...
            for i, d in enumerate(ok_days):
                ...
                for s in sel:
                    ...
                    if not (fu == fd and fu < 10**9):
                        ...
                        H = max(rem, 1)
                        if fu < fd and fu < 10**9:
                            b = min(3, max(0, math.ceil(fu * 4 / H) - 1))
                            hazard_state_all[s, :b + 1] = 0
                            hazard_state_all[s, b] = 1
                        elif fd < fu and fd < 10**9:
                            b = min(3, max(0, math.ceil(fd * 4 / H) - 1))
                            hazard_state_all[s, :b + 1] = 0
                            hazard_state_all[s, b] = 2
                        else:
                            hazard_state_all[s, :] = 0
                        # Fixed real-bar horizons avoid treating a short remaining
                        # path as four equally meaningful time points.
                        for k, horizon in enumerate((1, 2, 4, 8)):
                            if horizon <= rem:
                                if fu <= horizon:
                                    serial_state_all[s, k] = 1
                                elif fd <= horizon:
